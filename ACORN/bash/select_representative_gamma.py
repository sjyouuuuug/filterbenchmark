...
lb_bins = np.linspace(min(M_betas), max(M_betas), 5)          # 3 区间
gamma_bins = np.linspace(min(gammas), max(gammas), 4)        # 3 区间
...
representatives = (
    df.groupby(['m_bin', 'mb_bin', 'gamma_bin'])
...
representatives[['--M', '--M_beta', '--gamma']].to_csv(f"../data/result/ACORN/{dataset}/representative_parameters.csv", index=False)
