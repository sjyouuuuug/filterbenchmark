...
int main(int argc, char* argv[]) {
    std::cout
            << "====================\nSTART: running PARSE_CONTAINMENT_TEST --"
            << nthreads << "cores\n"
            << std::endl;
    ...
    int efc = 40;   // default is 40 // tripclick is 200
    ...
    const std::string gt_path =
            "./Datasets/" + dataset + "/overlap_graph_search_gt.txt";
    const std::string base_label_path = "./Datasets/" + dataset + "/" +
            dataset + "_label_overlap_12_base.txt";
    const std::string aq_path =
            "./Datasets/" + dataset + "/overlap_query_graph_search_label.txt";

    // Check if files exist
    if (file_exist(gt_path)) {
        std::cout << gt_path << " exists." << std::endl;
    } else {
        std::cerr << gt_path << " does not exist." << std::endl;
    }

    if (file_exist(base_label_path)) {
        std::cout << base_label_path << " exists." << std::endl;
    } else {
        std::cerr << base_label_path << " does not exist." << std::endl;
    }

    if (file_exist(aq_path)) {
        std::cout << aq_path << " exists." << std::endl;
    } else {
        std::cerr << aq_path << " does not exist." << std::endl;
    }
    ...
    { // load query vectors and attributes
        ...
        bool is_base = 0;
        std::string filename =
                "./Datasets/" + dataset + "/overlap_query_graph_search.fvecs";
    ...
    //     std::cout << "Create gamma=1 index in time: " << t2_gamma1 -
    //     t1_gamma1
    //               << std::endl;
    ...
    std::string hybrid_index_file;
    std::string hybrid_index_gamma1_file;

    std::stringstream filepath_stream;

    filepath_stream << "./tmp/" << dataset << "/hybrid"
                    << "_M=" << M << "_efc" << efc << "_Mb=" << M_beta
                    << "_gamma=" << gamma << ".json";

    hybrid_index_file = filepath_stream.str();

    std::stringstream filepath_stream2;
    filepath_stream2 << "./tmp/" << dataset << "/hybrid"
                     << "_M=" << M << "_efc" << efc << "_Mb=" << M_beta
                     << "_gamma=" << 1 << ".json";

    hybrid_index_gamma1_file = filepath_stream2.str();
