...
int main(int argc, char* argv[]) {
    ...
    const std::string gt_path =
            "./data/" + dataset + "/" + dataset + "_gt_equal_small.txt";
    const std::string base_label_path = "./data/" + dataset + "/label_base_small.txt";
    const std::string aq_path = "./data/" + dataset + "/" + dataset +
            "_query_equal_small.txt";
    ...
    // read in metadata
    metadata = load_single_label(base_label_path, N);
    ...
    { // load query vectors and attributes
        ...
        aq = load_single_label(aq_path, nq);
    ...
    // ACORN-1
    faiss::IndexACORNFlat hybrid_index_gamma1(d, M, 1, meta, 2 * M);
    ...
    // write hybrid index and partition indices to files
    {
        ...
        filepath_stream << "./tmp/" << dataset << "/hybrid" << "_M=" << M
                        << "_efc" << efc << "_Mb=" << M_beta
                        << "_gamma=" << gamma << ".json";
        ...
        filepath_stream2 << "./tmp/" << dataset << "/hybrid" << "_M=" << M
                         << "_efc" << efc << "_Mb=" << M_beta << "_gamma=" << 1
                         << ".json";
    ...
    // read query bitmap
    std::string query_bitmap_path =
            "./Datasets/" + dataset + "/query_bitmap.txt";
    std::ifstream query_bitmap_file(query_bitmap_path);
    if (!query_bitmap_file.is_open()) {
        std::cerr << "Failed to open query bitmap file: " << query_bitmap_path
                  << std::endl;
        return -1;
    }
    std::vector<uint32_t> query_bitmap;
    uint32_t num_queries;
    query_bitmap_file >> num_queries;
    query_bitmap.reserve(num_queries);
    for (uint32_t idx = 0; idx < num_queries; ++idx) {
        uint32_t query_bitmap_val;
        query_bitmap_file >> query_bitmap_val;
        query_bitmap.emplace_back(query_bitmap_val);
    }
    ...
    double pre_start = elapsed();
    std::vector<char> test_map(nq * N);
#pragma omp parallel for schedule(dynamic, 1)
    for (int xq = 0; xq < nq; xq++) {
        for (int xb = 0; xb < N; xb++) {
            test_map[xq * N + xb] = (bool)(metadata[xb] == aq[xq]);
        }
    }
    double pre_end = elapsed();
    prefilter_time += pre_end - pre_start;
    std::cout << "Prefilter time: " << prefilter_time << std::endl;
    std::cout << "nq: " << nq << "num_queries: " << num_queries << std::endl;
...
#pragma omp parallel for schedule(dynamic, 1)
        ...
        std::cout << "Search time: " << t2_x - t1_x2 << std::endl;
        std::cout << "Search time containing filter: " << t2_x - t1_x
                  << std::endl;

        std::cout << "QPS: " << nq / (t2_x - t1_x2) << std::endl;
        std::cout << "QPS containing filter: " << nq / (t2_x - t1_x)
                  << std::endl;

        qps_gamma.push_back(nq / (t2_x - t1_x2));
        qps_gamma_filter.push_back(nq / (t2_x - t1_x));
    ...
    for (auto& ef_search : search_lists) {
        ...
        qps_gamma1.push_back(nq / (t2_x - t1_x2));
        qps_gamma1_filter.push_back(nq / (t2_x - t1_x));
