...
int main(int argc, char* argv[]) {
    std::cout << "====================\nSTART: running CONTAINMENT_TEST --"
              << nthreads << "cores\n"
              << std::endl;
    ...
    int efc = 200;   // default is 40 // tripclick is 200
    ...
    { // load query vectors and attributes
        ...
        size_t d2;
        bool is_base = 0;
        std::string filename = "../data/" + dataset + "/" + dataset + "_query_and_small.fvecs";
    ...
    { // populating the database
        ...
        size_t nb, d2;
        bool is_base = 1;
        std::string filename = "../data/" + dataset + "/" + dataset + "_small.fvecs";
    ...
    std::string index_path = "../data/small_index_files/ACORN";

    std::string hybrid_index_file = index_path + "/" + dataset + 
                                "/hybrid_M=" + std::to_string(M) + 
                                "_Mb=" + std::to_string(M_beta) + 
                                "_gamma=" + std::to_string(gamma) + ".json";
    
    std::string hybrid_index_gamma1_file = index_path + "/" + dataset + 
                                        "/hybrid_M=" + std::to_string(M) + 
                                        "_Mb=" + std::to_string(M_beta) + 
                                        "_gamma=1.json";

    // auto& hybrid_index = *dynamic_cast<faiss::IndexACORNFlat*>(
    //         faiss::read_index(filepath.c_str()));
    // auto& hybrid_index_gamma1 = *dynamic_cast<faiss::IndexACORNFlat*>(
    //         faiss::read_index(filepath2.c_str()));
