...
int main(int argc, char* argv[]) {
    ...
    // const std::string gt_path = "../data/tripclick/tripclick_gt_or.txt";
    const std::string gt_path =
            "../data/" + dataset + "/" + dataset + "_gt_or.txt";
    const std::string base_label_path =
            "../data/" + dataset + "/label_base.txt";
    const std::string aq_path =
            "../data/" + dataset + "/" + dataset + "_query_or.txt";
    ...
    if (!file_exists(gt_path)) {
        fprintf(stderr, "Ground truth file does not exist: %s\n", gt_path.c_str());
        exit(1);
    }
    if (!file_exists(base_label_path)) {
        fprintf(stderr, "Base label file does not exist: %s\n", base_label_path.c_str());
        exit(1);
    }
    if (!file_exists(aq_path)) {
        fprintf(stderr, "Query file does not exist: %s\n", aq_path.c_str());
        exit(1);
    }
    ...
    { // load query vectors and attributes
        ...
        size_t d2;
        bool is_base = 0;
        // std::string filename = "../data/tripclick/tripclick_query_or.fvecs";
        std::string filename =
                "../data/" + dataset + "/" + dataset + "_query_or.fvecs";
    ...
    printf("[%.3f s] Index Params -- d: %ld, M: %d, N: %ld, gamma: %d, efc: %d\n",
           elapsed() - t0,
           d,
           M,
           N,
    ...
    std::cout << "Using efc = " << efc << " to build graph." << std::endl;
    ...
    { // populating the database
        ...
        size_t nb, d2;
        bool is_base = 1;
        // std::string filename = "../data/tripclick/tripclick_base.fvecs";
        std::string filename =
    ...
    // write hybrid index and partition indices to files
    {
        ...
        filepath_stream << "./tmp/" << dataset << "/hybrid_overlap"
        ...
        filepath_stream2 << "./tmp/" << dataset << "/hybrid_overlap"
                         << "_M=" << M << "_efc" << efc << "_Mb=" << M_beta
                         << "_gamma=" << 1 << ".json";
...
#pragma omp parallel for schedule(dynamic, 1)
    for (int xq = 0; xq < nq; xq++) {
        for (int xb = 0; xb < N; xb++) {
            // filter_ids_map[xq * N + xb] = 1 iff set intersection is not empty
            const auto& set1 = metadata[xb];
            const auto& set2 = aq[xq];

            // iterate over the smaller set and use binary search on the
            // larger set
    ...
    const std::string csv_path = "./overlap_results/" + dataset +
            "/M=" + std::to_string(M) + "_MB=" + std::to_string(M_beta) +
            ".csv";
    std::ofstream csv_file(csv_path);
    if (!csv_file) {
        std::cerr << "Error creating CSV file: " << csv_path << std::endl;
        return 0;
    }
    ...
    std::cout
            ...
            << std::endl;
    csv_file << "L,QPS,QPS(filter),dist_cmp,recall@10" << std::endl;
    ...
    for (int i = 0; i < search_lists.size(); i++) {
        ...
        csv_file << search_lists[i] << "," << qps_gamma[i] << ","
                 << qps_gamma_filter[i] << "," << dist_cmps_gamma[i] << ","
                 << recall_at_10_gamma[i] << std::endl;
    }
    csv_file.close();
