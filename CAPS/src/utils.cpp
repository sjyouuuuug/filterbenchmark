...
int argparser(int argc, char** argv, string* basepath, string* labelpath, string* querypath, string* queryAttripath, string* indexpath, string* GTpath, string* output_csv, size_t* nc, string* algo, int* mode, size_t* buffer_size)
{
    if (argc < 8){
        std::clog<<"Usage: "<<std::endl; 
        std::clog<<"./index <data> <properties> <queries> <queryProperties> <index> <groundtruth> <output_csv>";
        std::clog<<" [--Nc num_clusters] [--Algo method] [--mode method_version] [--Bf BufferSize]"<<std::endl;
        ...
        std::clog<<"\t index: folder for the index. Example : sift1024blissMode1, sift1024kmeans"<<std::endl;
        std::clog<<"\t groundtruth: file for the groundtruth."<<std::endl;
        std::clog<<"\t output_csv: file for the output csv."<<std::endl;
    ...
    *indexpath = (string(argv[5]));
    *GTpath = (string(argv[6]));
    *output_csv = (string(argv[7]));
    *nc = std::atoi(argv[8]);
    *algo = std::string(argv[9]);
    *mode = std::atoi(argv[10]);
    *buffer_size = std::atoi(argv[11]);
    return 0;
}
...
double RecallAtK(int* answer, int* guess, size_t k, size_t nq){
    ...
    for (int i=0;i<nq;i++){
        ...
        // for(int j = 0; j < k; j++)
        // {
        //     std::cout << *(answer + k*i + j) << " ";
        // }
        // std::cout << std::endl;
        // for(int j = 0; j < k; j++)
        // {
        //     std::cout << *(guess + k*i + j) << " ";
        // }
        // std::cout << std::endl;
