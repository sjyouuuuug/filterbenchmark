...
const std::vector<int> search_lists = {50, 150, 250, 400, 600, 800, 1200, 1500, 2000};
...
//     // 限制最多检查的数量
//     size_t max_check = query_nns.size();
//     if (k > 0) {
//         max_check = std::min(max_check, query_nns.size() / (k * 2));
//         if (max_check == 0) max_check = query_nns.size(); // 防止k太大导致为0
//     }

//     auto match = [&](size_t i) -> bool {
//         const auto idx = query_nns[i];
//         if (idx >= metadata.size()) return false;
//         const auto& candidate_labels = metadata[idx];

//         if (scenario == "and") {
//             return std::includes(
//                 candidate_labels.begin(), candidate_labels.end(),
//                 query_labels.begin(), query_labels.end()
//             );
//         } else if (scenario == "or") {
//             if (query_labels.size() < candidate_labels.size()) {
//                 for (int lbl : query_labels)
//                     if (candidate_labels.count(lbl)) return true;
//                 return false;
//             } else {
//                 for (int lbl : candidate_labels)
//                     if (query_labels.count(lbl)) return true;
//                 return false;
//             }
//         } else if (scenario == "equal") {
//             return candidate_labels == query_labels;
//         } else {
//             std::cerr << "Invalid scenario: " << scenario << std::endl;
//             exit(EXIT_FAILURE);
//         }
...
int main(int argc, char* argv[]) {
    std::cout << "==================== START: running SEARCH_HNSW_INDEX --"
    ...
    if (argc != 14) {
        std::cerr
                << "Usage: " << argv[0]
                << "<dataset> <M> <efc> <index_path> <scenario> <output_path_prefix> "
                << "<base_file> <base_label> <query_file> <query_label> <gt_path> <k> <N>"
                << std::endl;
        return 1;
    }
    ...
    M = atoi(argv[2]);
    efc = atoi(argv[3]);
    const std::string index_path = argv[4];
    const std::string scenario = argv[5];
    const std::string output_path_prefix = argv[6];
    const std::string base_file = argv[7];
    const std::string base_label = argv[8];
    const std::string query_file = argv[9];
    const std::string query_label = argv[10];
    const std::string gt_path = argv[11];
    k = atoi(argv[12]);
    N = atoi(argv[13]);

    const std::string output_path = output_path_prefix +
            "/M=" + std::to_string(M) + "_efc=" + std::to_string(efc) + ".txt";

    const std::string output_csv = output_path_prefix +
            "/M=" + std::to_string(M) + "_efc=" + std::to_string(efc) +
            "_result.csv";
    ...
    filepath_stream << index_path << "/" << dataset << "/M=" << M
                    << "_efc=" << efc << ".json";
    ...
    auto& hnsw_index =
            *dynamic_cast<faiss::IndexHNSWFlat*>(faiss::read_index(cfile_path));
    std::vector<double> qps_list, recall_at_k_list, dist_cmps_list;

    std::cout << "=== Performing search with efc=" << efc
              << " ===" << std::endl;

    int result_list = k * 10;
    auto max_possible_depth = N / 2;
        ...
        for (int q = 0; q < nq; ++q) {
            ...
            while (true) {
                ...
                auto [nns, dis, search_time] =
                        perform_single_search(hnsw_index, cur_xq, depth);
                // auto filtered = post_filter(nns, metadata, aq[q], scenario, k);
